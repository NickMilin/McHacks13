...
export function SearchRecipes() {
  ...
  const handleImportRecipe = async () => {
    ...
    try {
      ...
      setError(
        "Failed to import recipe. Make sure the backend server is running on port 5001.",
      );
    } finally {
  ...
  const handleAddToShoppingList = (recipe) => {
    const missingIngredients = checkPantryAvailability(
      recipe.ingredients,
    ).missing;
          ...
          <Card className="overflow-hidden">
            ...
            <CardContent className="space-y-6">
              ...
              {importedRecipe.ingredients.length > 0 && (
                <div>
                  {(() => {
                    const { available, missing } = checkPantryAvailability(
                      importedRecipe.ingredients,
                    );
                    const matchPercentage = Math.round(
                      (available.length / importedRecipe.ingredients.length) *
                        100,
                    );
              ...
              <div>
                ...
                {(() => {
                  ...
                  const availableNames = available.map((a) =>
                    a.name.toLowerCase(),
                  );
                      ...
                      <div key={groupName} className="mb-4">
                        ...
                        <ul className="space-y-1.5">
                          {ingredients.map((ing, i) => {
                            const inPantry = availableNames.includes(
                            ...
                            return (
                              <li key={i} className="flex items-start gap-2">
                                ...
                                <span
                                  className={
                                    inPantry ? "text-muted-foreground" : ""
                                  }
                                >
              ...
              {/* Missing Ingredients */}
              {checkPantryAvailability(importedRecipe.ingredients).missing
                .length > 0 && (
            ...
            <CardFooter className="flex gap-2">
              <Button
                variant="outline"
                className="flex-1"
                onClick={() => handleSaveRecipe(importedRecipe)}
              >
            ...
            <div className="flex flex-col items-center text-center">
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${
                  notification.type === "cooking" ||
                  notification.type === "cooking-saved"
                    ? "bg-green-100"
                    : "bg-primary/10"
                }`}
              >
